...
function App() {
  ...
  try {
    const response = await fetch("https://salimshakeel-video-summarization-backend.hf.space/summarize", {
      method: "POST",
      body: formData,
    });

    const text = await response.text(); // Read raw text from response

    if (!response.ok) {
      console.error("Server Error Response:", text); // Log server error
      throw new Error(text); // Show raw backend error in alert
    }

    const data = JSON.parse(text); // Now safely parse JSON
    const fullUrl = `https://salimshakeel-video-summarization-backend.hf.space${data.summary_video_url}`;
  ...
  link.setAttribute("download", `summary_${video.name}`);
